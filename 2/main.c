...
static int led_board_init(void)
{
	...
	GPIO5_GDIR = ioremap(RegData[6], RegData[7]);
	GPIO5_DR = ioremap(RegData[8], RegData[9]);
...
static __init int led_init(void)
{
	...
	led_node = of_find_compatible_node(NULL, NULL, "imx6ull,led");
	if (led_node == NULL) {
		goto of_error;
	}
	of_property_read_u32_array(led_node, "reg", RegData, 10);
